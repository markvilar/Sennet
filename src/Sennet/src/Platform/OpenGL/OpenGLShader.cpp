...
static GLenum ShaderTypeFromString(const std::string& type)
{
    ...
    SENNET_CORE_ERROR("Could not convert shader type '{0}'", type);
    SENNET_CORE_ASSERT(false, "Unknown shader type!");
    return 0;
}
...
std::string OpenGLShader::ReadFile(const std::string& filepath)
{
    ...
    {
        SENNET_CORE_ERROR("Could not open file '{0}'", filepath);
    }
    ...
    const std::unordered_map<GLenum, std::string>& shaderSources)
{
    ...
    for (auto& kv : shaderSources)
    {
        ...
        if (isCompiled == GL_FALSE)
        {
            ...
            SENNET_CORE_ERROR("{0}", infoLog.data());
            SENNET_CORE_ASSERT(false, "Shader compilation failure!")
            break;
        }
    ...
    if (isLinked == GL_FALSE)
    {
        ...
        SENNET_CORE_ERROR("{0}", infoLog.data());
        SENNET_CORE_ASSERT(false, "Shader link failure!");
        return;
    }
