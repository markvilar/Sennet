...
bool is_key_pressed(const Window& window, const KeyCode keyCode)
{
    auto native_window = static_cast<GLFWwindow*>(window.get_native_window());
    auto state = glfwGetKey(native_window, static_cast<int32_t>(keyCode));
    return state == GLFW_PRESS || state == GLFW_REPEAT;
}

bool is_mouse_button_pressed(const Window& window, MouseCode button)
{
    auto native_window = static_cast<GLFWwindow*>(window.get_native_window());
    auto state =
