...
{
    return ImGui::GetCurrentContext() ? static_cast<ImGui_ImplOpenGL3_Data*>(
...
bool ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
    ...
    IM_ASSERT(static_cast<int>(strlen(glsl_version)) + 2
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (bd->GlVersion >= 330)
    {
        glGetIntegerv(GL_SAMPLER_BINDING,
            reinterpret_cast<GLint*>(&last_sampler));
    }
    else
    {
        last_sampler = 0;
            ...
            else
            {
                    ...
                    && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    glBindTexture(GL_TEXTURE_2D,
                        static_cast<GLuint>(
                            reinterpret_cast<intptr_t>(pcmd->GetTexID())));
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_VTX_OFFSET
                    if (bd->GlVersion >= 320)
    ...
    glViewport(last_viewport[0],
        last_viewport[1],
        last_viewport[2],  // (GLsizei)
        last_viewport[3]); // (GLsizei)
    glScissor(last_scissor_box[0],
        last_scissor_box[1],
        last_scissor_box[2],  // (GLsizei)
        last_scissor_box[3]); // (GLsizei)
