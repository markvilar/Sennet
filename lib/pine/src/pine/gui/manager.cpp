...
std::unique_ptr<IO> create_io() { return std::make_unique<IO>(); }
...
void Context::init() const {
    ...
    auto native_window =
        static_cast<GLFWwindow*>(instance->get_native_window());
...
bool IO::load_font(const std::filesystem::path& filepath,
    const float pixel_size) const {
    auto& io = ImGui::GetIO();
    io.Fonts->AddFontFromFileTTF(filepath.string().c_str(),
        pixel_size,
        nullptr,
        io.Fonts->GetGlyphRangesCyrillic());
    return true;
